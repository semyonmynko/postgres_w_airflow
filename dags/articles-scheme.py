...
def init_schema():
    ...
    class Articles(base_class):
        __tablename__ = 'articles'
        __table_args__ = (
            UniqueConstraint('title', 'journal_rubric_id', 
                             name='uix_title_journal'),
            {'schema': 'public'}
        )
                        
        id = Column('id', Integer, primary_key=True, autoincrement=True)
        title = Column('title', String)
        journal_rubric_id = Column('journal_rubric_id', Integer)
        

    if not db_engine.dialect.has_table(db_engine, Articles.__tablename__):
        Articles.metadata.create_all(bind=db_engine)
        db_session.commit()
