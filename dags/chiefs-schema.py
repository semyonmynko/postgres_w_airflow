...
def init_schema():
    ...
    class Chiefs(base_class):
        __tablename__ = 'chiefs'
        __table_args__ = (
            UniqueConstraint('measure_id', 'person_id', 
                             name='uix_measure_person'),
            {'schema': 'public'}
        )
    ...
    if not db_engine.dialect.has_table(db_engine, Chiefs.__tablename__):
        Chiefs.metadata.create_all(bind=db_engine)
        db_session.commit()
